// Vercel serverless function — this IS the registered WHOOP Redirect URI.
// WHOOP sends the browser here with ?code=..., we exchange it for tokens
// server-side (client secret never touches the browser), then 302 the user
// to health.html with the tokens in the URL fragment (never sent to any
// server on subsequent requests) so the frontend can pick them up.
//
// Env vars required (Vercel project settings):
//   WHOOP_CLIENT_ID, WHOOP_CLIENT_SECRET, WHOOP_REDIRECT_URI
//   (WHOOP_REDIRECT_URI must equal this function's own deployed URL,
//   e.g. https://awhoop.vercel.app/api/whoop-callback)
